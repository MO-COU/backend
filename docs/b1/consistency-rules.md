...
| 규칙 수 | **9종** (`R1`~`R9`). 이 중 `R7`은 검출력 증명이라 실행 목록에는 8종이 오른다 |
...
| `R9` | `ISSUE_SEQUENCE_MISMATCH` | Redis 예약 순번·잔여 재고가 총재고와 어긋남 (4항목) | **항목별 합** | 0 |

**`checked_count`는 규칙마다 세는 단위가 다르다.** `R5`가 발급 건수보다 큰 것, `R6`이 300만을 넘는 것은 정상이다. 항목·참조가 여럿이라 각각을 더하기 때문이다.

**`R9`의 실측이 0인 것도 정상이다.** 더미데이터는 Redis를 거치지 않아 순번이 없다. 부하 테스트를 거친 발급 건에만 값이 생긴다.
...
| `R9` `ISSUE_SEQUENCE_MISMATCH` | **항목마다 다름** | 카운터·한쪽만 채움은 **행**, 중복·구멍은 **쿠폰** 단위 |
...
### R9. `ISSUE_SEQUENCE_MISMATCH` — 예약 순번 정합성

Redis가 예약 성공 순간에 확정한 **순번**과 **차감 후 잔여 재고**가 DB까지 온전히 왔는가.

검사 대상은 **두 값이 채워진 발급 건뿐**이다. 더미데이터 300만 건은 Redis를 거치지 않고 직접 적재되므로 두 컬럼이 `NULL`이고, 여기서 `NULL`은 결측이 아니라 **"검사 대상 아님"** 을 뜻한다.

#### 무엇을 증명하고 무엇을 증명하지 않나

| | |
| --- | --- |
| 증명한다 | Redis가 정한 결과가 DB까지 온전히 왔는가 |
| 증명하지 않는다 | "선착순이 공정했는가" — 이는 Lua 싱글 스레드 원자 실행이라는 **구조**가 보장하며, 데이터를 사후에 검사해 밝힐 성질이 아니다 |

DB 안에는 대조할 순서가 없다. `issued_at`은 초 단위라 동률이 수천 건이고, `coupon_issue_id`는 컨슈머가 Stream을 배치로 적재한 순서라 예약 순서와 다른 것이 정상이다. 그래서 **Lua가 같은 원자 실행 안에서 만든 두 독립 카운터**(`INCR` 순번, `DECR` 재고)를 DB가 원래 갖고 있던 `total_quantity`와 맞춰본다.
...
| 코드 | 단위 | 조건 |
| --- | --- | --- |
| `SEQUENCE_STOCK_DIVERGED` | 발급 건 | 합 < 총재고, 또는 순번 순으로 합이 감소 |
| `SEQUENCE_HALF_WRITTEN` | 발급 건 | 두 컬럼 중 **한쪽만** 채워짐 (양방향) |
| `SEQUENCE_DUPLICATED` | 쿠폰 | `COUNT(DISTINCT 순번) ≠ 발급 건수` |
| `SEQUENCE_GAP` | 쿠폰 | `MAX(순번) ≠ 발급 건수` |

`SEQUENCE_HALF_WRITTEN`은 `(순번 IS NULL) <> (잔여 IS NULL)`로 판정한다. `순번 IS NOT NULL`로 대상을 좁히면 **순번만 빠진 행이 필터에 걸러져 조용히 통과한다.**
...
| 항목 | 값 |
| --- | --- |
| `checked_count` | 항목별 대상 수의 합 (발급 건 단위 2항목 + 쿠폰 단위 2항목) |
| `target_type` | `COUPON_ISSUE`(행 단위) / `COUPON`(쿠폰 단위) |
| `detail` | 코드와 함께 어긋난 값 |

한 쿠폰이 두 항목을 어기면 위반은 2건이다. `R5`와 같은 방식이며, 항목별 건수가 그 자체로 조사 단서이기 때문이다.
