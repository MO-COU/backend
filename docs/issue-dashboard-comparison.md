...
쿠폰 발급 결과를 관찰하는 두 가지 방식을 비교한다. 현재 #124는 Redis 원본 카운터를 직접 조회하는 개발자용 화면이고, #125는 Prometheus와 Grafana를 이용한 시계열 관측 환경을 별도로 설계·구현하는 범위다.
...
| 조회 방식 | 브라우저가 관리 API를 5초마다 호출 | Grafana가 PromQL로 Prometheus 시계열 조회 |
...
현재 브랜치에서는 Redis 방식만 구현한다. Prometheus registry 의존성, `/actuator/prometheus` 노출, Prometheus 설정, Grafana provisioning과 대시보드 JSON은 #125에서 결정하고 추가한다.
