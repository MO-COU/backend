...
B2는 발급이 끝난 쿠폰의 생명주기를 관리한다. 쿠폰 발급과 최초 `UNISSUED → ISSUED` 이력 기록은 B2 범위 밖이며, B2는 이후의 사용·만료 전이와 상태 이력을 처리한다.
...
| 사용 알림 | 최초 사용 전이가 커밋된 뒤 회원에게 `USED` 알림 발송 |

일반 조회와 쿠폰 발급은 B2 범위가 아니다.

## 상태와 전이

`미발급`은 `coupon_issue` 행이 없는 상태다. `UNISSUED`는 `coupon_issue.status`에 저장하는 상태가 아니라, 최초 발급 이력에서 발급 전 상태를 나타내는 이력 전용 값이다. 발급 건의 저장 상태는 `ISSUED`, `USED`, `EXPIRED`만 사용하며 `USED`, `EXPIRED`는 최종 상태다.

```text
행 없음 ──발급──> ISSUED ──사용──> USED
                         └─만료──> EXPIRED

최초 발급 이력: UNISSUED ──발급──> ISSUED
```
...
상태 이력은 전이 전·후 상태를 모두 명시한다. 최초 발급 이력도 예외가 아니며, `from_status`에 `UNISSUED`, `to_status`에 `ISSUED`를 기록한다. 이 값은 발급 전 `coupon_issue` 행이 없었다는 사실을 이력에서 명시적으로 표현할 뿐, 미발급 건을 별도 행으로 생성하지 않는다.

| 전이 | `from_status` | `to_status` | `coupon_issue` 상태 |
| --- | --- | --- | --- |
| 최초 발급 | `UNISSUED` | `ISSUED` | `ISSUED` 행 생성 |
| 쿠폰 사용 | `ISSUED` | `USED` | `USED` |
| 쿠폰 만료 | `ISSUED` | `EXPIRED` | `EXPIRED` |

## 최초 발급 이력

발급 트랜잭션은 `coupon_issue` 생성과 `UNISSUED → ISSUED` 최초 이력 기록을 함께 처리한다. 최초 이력의 멱등성 키는 `ISSUE:{couponIssueId}`를 사용한다.
...
- `ISSUED → USED` 조건부 갱신에 성공한 최초 요청만 사용 완료 이벤트를 발행한다.
- 알림은 쿠폰 사용 트랜잭션이 커밋된 뒤 `NotificationSender`를 통해 회원에게 발송한다.
- 같은 멱등성 키의 재시도와 동시 요청의 패자는 기존 결과만 반환하고 알림 이벤트를 다시 발행하지 않는다.
- 쿠폰 사용 상태 또는 이력 저장이 롤백되면 알림을 호출하지 않는다.
- 알림 기록은 별도 트랜잭션에서 처리하며, 실패하더라도 이미 커밋된 쿠폰 사용 결과는 유지한다.
- 현재 Mock 알림은 자동 재시도나 전달 보장을 제공하지 않으므로 실패 로그를 운영자가 확인해야 한다.
...
```text
사용 요청 → 멱등성 이력 확인 → 조건부 USED 갱신 → USED 이력 저장 → 사용 완료 이벤트 발행 → 커밋 → USED 알림
...
- 모든 발급 건은 `UNISSUED → ISSUED` 이력을 가지며, 최종 상태 건은 후속 이력 한 건을 더 가진다.
...
- 재시도와 동시 사용 요청에서도 `USED` 알림은 최초 성공 건에 대해 1건만 기록된다.
- 사용 상태 또는 이력 저장 실패 시 `USED` 알림이 기록되지 않는다.
- 알림 저장 실패가 이미 커밋된 쿠폰 사용 상태와 이력을 롤백하지 않는다.
