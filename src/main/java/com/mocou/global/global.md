...
현재 정의된 코드: `INVALID_INPUT`, `METHOD_NOT_ALLOWED`, `COUPON_NOT_FOUND`, `SOLD_OUT`, `DUPLICATE`, `NOT_MEMBER`, `NOT_OPEN_YET`, `ISSUE_NOT_FOUND`, `IDEMPOTENCY_CONFLICT`, `INVALID_STATE_TRANSITION`, `COUPON_EXPIRED`, `SYSTEM_ERROR`, `COUPON_ISSUE_NOT_READY`, `ISSUE_CLOSED`

팀원들이 더 추가할 예정 : ex)`SERVICE_UNAVAILABLE`
