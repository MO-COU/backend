-- 호출 전 @run_key, @batch_count를 설정한다. @race_time은 MySQL 기준 15초 뒤로 고정한다.
...
WITH RECURSIVE sequence AS (SELECT 1 AS number UNION ALL SELECT number + 1 FROM sequence WHERE number < @batch_count)
SELECT CONCAT('perf-', @run_key, '-race-', number, '@example.invalid'), CONCAT('perf-', number), CONCAT('010-', LPAD(number, 8, '0')) FROM sequence;
INSERT INTO coupon_issue (coupon_id, member_id, status, issued_at, expires_at)
SELECT @coupon_id, member_id, 'ISSUED', CURRENT_TIMESTAMP - INTERVAL 1 DAY, @race_time
FROM member WHERE email LIKE CONCAT('perf-', @run_key, '-race-%@example.invalid');
...
SELECT @coupon_id AS coupon_id, @race_time AS race_time;
