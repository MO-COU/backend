...
const remainderRequests = [0, 1, 3];
const EXPECTED_REQUESTS = Math.floor(VUS / 3) * 6 + remainderRequests[VUS % 3];
...
export const options = {
  ...
  thresholds: {
    http_reqs: [`count==${EXPECTED_REQUESTS}`],
    http_req_duration: ['p(95)<2000'],
    http_req_failed: ['rate<0.01'],
    checks: ['rate==1'],
    issue_success_202: [`count==${Math.min(VUS, EXPECTED_STOCK)}`],
...
export default function () {
  ...
  const requestCount = 1 + ((vuId - 1) % 3);

  // 1·2·3회 균등 배분함. 총 요청 수 고정.
  for (let attempt = 0; attempt < requestCount; attempt += 1) {
    const res = http.post(
      ...
      JSON.stringify({ memberId }),
      {
        ...
        tags: { name: 'flash-sale-repeat-one-to-three' },
